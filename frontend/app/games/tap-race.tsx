...
export default function TapRace() {
  ...
  useEffect(() => {
    if (phase === "done") earnPoints({ gameId: "tap", points: taps * PER_TAP, title: "Tap Race" });
  }, [phase]);
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.body, { paddingBottom: insets.bottom + 24 }]}>
        <View style={styles.statsRow}>
          ...
          <View style={styles.statBox}>
            <Text style={[styles.statValue, { color: left <= 3 && phase === "running" ? colors.error : colors.onSurface }]}>{left}s</Text>
      ...
      <GameResult
        visible={phase === "done"}
        title="Time's up!"
        subtitle={`You tapped ${taps} times`}
        points={taps * PER_TAP}
...
const useStyles = makeStyles((colors) => ({
  ...
  statBox: { flex: 1, backgroundColor: colors.surfaceSecondary, borderRadius: 16, padding: 20, alignItems: "center", borderWidth: 1, borderColor: colors.border },
  statValue: { color: colors.onSurface, fontSize: 34, fontWeight: "900" },
  statLabel: { color: colors.muted, fontSize: 13, marginTop: 4 },
  tapBtn: { marginTop: 60, width: 220, height: 220, borderRadius: 110, backgroundColor: colors.accentTap, alignItems: "center", justifyContent: "center" },
  ...
  bolt: { width: 110, height: 110, borderRadius: 32, backgroundColor: colors.accentTapSoft, alignItems: "center", justifyContent: "center" },
  ...
  startBtn: { marginTop: 32, backgroundColor: colors.brandPrimary, borderRadius: 40, paddingVertical: 18, paddingHorizontal: 64 },
